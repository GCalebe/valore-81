 ...
 */
export const formatMemoryForN8n = (
  memory: N8nChatMemory,
): Record<string, any> => {
 ...
 */
export const formatN8nObjectToMemory = (
  n8nObject: Record<string, any>,
): Omit<N8nChatMemory, "id"> => {
  // Verificar se o objeto tem a estrutura esperada
  if (!n8nObject.sessionId || !n8nObject.message) {
    throw new Error(
      "Objeto n8n inválido: sessionId e message são obrigatórios",
    );
  }
...
export const generateContextSummary = (
  ...
  const messageContents = memories
    .map((memory) => {
      let content = "";
