...
export const generateFictitiousConversation = (contact: Contact): Contact => {
  ...
  if (contact.clientType && messagesByClientType[contact.clientType as keyof typeof messagesByClientType]) {
    const typeMessages = messagesByClientType[contact.clientType as keyof typeof messagesByClientType];
    if (Math.random() > 0.5) {
  ...
  if (contact.kanbanStage && messagesByStage[contact.kanbanStage as keyof typeof messagesByStage]) {
    const stageMessages = messagesByStage[contact.kanbanStage as keyof typeof messagesByStage];
    if (Math.random() > 0.3) {
  ...
  if (contact.kanbanStage === 'Negociação' || contact.kanbanStage === 'Preparando proposta') {
