...
type TabValue = 'principal' | 'stats' | 'custom' | 'more';
...
const ClientInfoTabs = ({ clientData, dynamicFields, onFieldUpdate }: ClientInfoTabsProps) => {
  ...
  const getActiveTabLabel = () => {
    return tabOptions.find(tab => tab.value === activeTab)?.label || 'Principal';
  };
        ...
        <DropdownMenuContent className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
          {tabOptions.map((tab) => (
            <DropdownMenuItem
              ...
              className={`cursor-pointer ${activeTab === tab.value ? 'bg-gray-100 dark:bg-gray-700' : ''}`}
            >
              {tab.label}
            </DropdownMenuItem>
          ))}
