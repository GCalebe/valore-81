...
import { MessageCircle, Calendar, Clock, ArrowDown, ArrowUp } from 'lucide-react';
...
const InteractionTimeline: React.FC<InteractionTimelineProps> = ({ sessionId, maxHeight = '300px' }) => {
  ...
  const formatTimelineItems = (history: N8nChatHistory[]): TimelineItem[] => {
    return history.map((item) => {
      ...
      const messageObj = typeof item.message === 'string' ? JSON.parse(item.message) : item.message;
      ...
      if (messageObj.type === 'status_update') {
        ...
        sender = messageObj.sender || messageObj.from === 'user' ? 'user' : 'bot';
      }
  ...
  const renderTimelineItem = (item: TimelineItem) => {
    ...
    return (
      <div key={item.id} className="mb-4 relative pl-6 border-l-2 border-gray-200 dark:border-gray-700">
        <div className="absolute -left-1.5 mt-1.5">
          {item.type === 'message' ? (
            <Badge className={item.sender === 'user' ? 'bg-blue-500' : 'bg-green-500'}>
