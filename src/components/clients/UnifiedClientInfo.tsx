...
const UnifiedClientInfo: React.FC<UnifiedClientInfoProps> = ({
  ...
  const renderField = (label: string, value: any, type: 'text' | 'badge' | 'money' = 'text') => {
    if (!value && value !== 0) {
      value = 'Não informado';
    }
  ...
  const renderCommercialInfo = () => (
    <Card className={compact ? 'p-3' : 'p-4'}>
      ...
      <CardContent className="p-0 space-y-2">
        ...
        {renderField('Etapa da Consulta', clientData?.consultationStage, 'badge')}
  ...
  return (
    <div className="space-y-4">
      <Tabs defaultValue="basic" className="w-full">
        <TabsList className="w-full grid" style={{ gridTemplateColumns: `repeat(${showTabs.length}, 1fr)` }}>
          {showTabs.includes('basic') && <TabsTrigger value="basic">Básico</TabsTrigger>}
          {showTabs.includes('commercial') && <TabsTrigger value="commercial">Comercial</TabsTrigger>}
          {showTabs.includes('utm') && <TabsTrigger value="utm">UTM</TabsTrigger>}
          {showTabs.includes('custom') && <TabsTrigger value="custom">Personalizado</TabsTrigger>}
          {showTabs.includes('docs') && <TabsTrigger value="docs">Documentos</TabsTrigger>}
        </TabsList>
