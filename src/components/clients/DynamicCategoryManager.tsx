...
const DynamicCategoryManager = ({ tabName, categories, onCategoriesChange }: DynamicCategoryManagerProps) => {
      ...
      case 'single_select':
        return (
          <Select 
            ...
            onValueChange={(val) => updateCategoryValue(category.id, val === 'none' ? null : val)}
          >
            <SelectTrigger>
              <SelectValue placeholder={`Selecione ${category.name.toLowerCase()}`} />
            </SelectTrigger>
            <SelectContent>
            ...
            {category.options?.map((option) => (
              <label key={option} className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      updateCategoryValue(category.id, [...selectedValues, option]);
                    } else {
                      updateCategoryValue(category.id, selectedValues.filter((v: string) => v !== option));
                    }
                  }}
        ...
        {isAddingCategory && (
          <Card className="border-green-200">
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                <Label htmlFor="category-type" className="text-xs">Tipo de Campo</Label>
                <Select
              ...
              {(newCategory.type === 'single_select' || newCategory.type === 'multi_select') && (
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    <Input
                      ...
                      className="text-sm"
                    />
                    <Button onClick={addOption} type="button" variant="outline" size="sm">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="text-xs">
                      {category.type === 'text' ? 'Texto' : 
                       category.type === 'single_select' ? 'Seleção Única' : 'Seleção Múltipla'}
                    </Badge>
                  </div>
                  <Button
                    type="button"
