    ...
    const fetchUTMData = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          const totalCampaigns = new Set(data.map(item => item.utm_campaign).filter(Boolean)).size;
          ...
          const sourcesMap = data.reduce((acc: Record<string, number>, item) => {
            if (item.utm_source) {
          ...
          const campaignsMap = data.reduce((acc: Record<string, number>, item) => {
          ...
          {utmData.length === 0 ? (
            <Card>
              <CardContent className="pt-6">
                <p className="text-center text-gray-500">Nenhum dado UTM encontrado para este cliente.</p>
              </CardContent>
            </Card>
          ) : (
            <>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium">Principais Fontes</CardTitle>
                    <CardDescription>De onde vieram as interações</CardDescription>
                  </CardHeader>
                  <CardContent>
                      ...
                      <div className="space-y-2">
                        {metrics.topSources.map((source, index) => (
                          <div key={index} className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-medium">{source.count} interações</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-sm text-gray-500">Nenhuma fonte encontrada</p>
                    )}
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium">Principais Campanhas</CardTitle>
                      ...
                      <div className="space-y-2">
                        {metrics.topCampaigns.map((campaign, index) => (
                          <div key={index} className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-medium">{campaign.count} interações</span>
                          </div>
                        ))}
                      </div>
                    ) : (
        ...
        <TabsContent value="configuracoes" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Parâmetros UTM para este cliente</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {utmData.length === 0 ? (
                    ...
                    Array.from(new Set(utmData.map(item => 
                    ...
                    .map((uniqueKey, index) => {
                      ...
                      const item = utmData.find(data => 
                        `${data.utm_source}-${data.utm_medium}-${data.utm_campaign}-${data.utm_content}-${data.utm_term}` === uniqueKey
                      );
                      
                      if (!item) return null;
                      
                      // Nome amigável é uma combinação de source e campaign
                      const friendlyName = `${item.utm_source || 'Direto'} - ${item.utm_campaign || 'Sem campanha'}`;
                      ...
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <Badge variant="outline" className="bg-pink-50 text-pink-600">
                              {item.utm_term || '-'}
                            </Badge>
                          </TableCell>
                          <TableCell>
