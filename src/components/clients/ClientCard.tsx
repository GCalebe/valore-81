...
import { Edit2, MessageSquare, Phone, Mail, ShipWheel, Tag, AlertCircle, DollarSign, Target } from 'lucide-react';
    ...
    >
      ...
      >
        <CardContent className={cn("p-3 text-sm", isCompact && "py-1.5")}>
          ...
          {/* Tags */}
          {displayConfig.showTags && contact.tags && contact.tags.length > 0 && (
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 mb-2 text-xs">
              {contact.budget && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-gray-600 dark:text-gray-300">R$ {contact.budget.toFixed(2)}</span>
                </div>
              )}
              {contact.clientObjective && (
                <div className="flex items-center gap-1">
                  <Target className="h-3 w-3 text-gray-500" />
                  <span className="text-gray-600 dark:text-gray-300 truncate">{contact.clientObjective}</span>
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex flex-col gap-1 mb-2">
            ...
            {!isCompact && contact.email && (
              <div className="flex items-center gap-1 text-xs">
                ...
                <span className="text-gray-600 dark:text-gray-300 truncate">{contact.email}</span>
              </div>
            )}
          </div>
