...
import { Filter, X, Plus, Save, ChevronDown, ChevronUp, Trash2 } from 'lucide-react';
...
const FilterRuleComponent = ({
  ...
  const operators = selectedField ? operatorsByType[selectedField.type as keyof typeof operatorsByType] : [];
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-2 p-2 bg-muted/30 rounded-md">
      <Select
        ...
        onValueChange={(value) => {
          ...
          onUpdate(rule.id, {
            ...
            operator: operatorsByType[field?.type as keyof typeof operatorsByType][0].id,
            value: '',
          });
        }}
      >
      ...
      {selectedField?.type === 'select' ? (
        ...
        <Input
          value={rule.value}
          onChange={(e) => onUpdate(rule.id, { ...rule, value: e.target.value })}
          placeholder="Valor"
          className="w-[140px]"
        />
      )}
...
const FilterGroupComponent = ({
  ...
  const addRule = () => {
    const newRule: FilterRule = {
      ...
      operator: operatorsByType[clientProperties[0].type as keyof typeof operatorsByType][0].id,
      value: '',
    };
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Button
            ...
            className={`text-xs font-medium ${group.condition === 'AND' ? 'bg-blue-100 hover:bg-blue-200 text-blue-700' : 'bg-orange-100 hover:bg-orange-200 text-orange-700'} border-none`}
          >
            {group.condition}
          </Button>
          <span className="text-sm text-muted-foreground">
            {group.condition === 'AND' ? 'Todas as condições devem ser verdadeiras' : 'Qualquer condição pode ser verdadeira'}
          </span>
        </div>
        {!isRoot && (
          <Button
      ...
      {savedFilters.length === 0 ? (
        ...
        <div className="space-y-2">
          {savedFilters.map(filter => (
            <div key={filter.id} className="flex items-center justify-between p-2 bg-muted/30 rounded-md">
...
const FilterDialog = ({
  ...
  const [savedFilters, setSavedFilters] = useState<{ id: string; name: string; filter: FilterGroup }[]>([]);
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="w-full sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader className="mb-4">
          ...
          <DialogDescription>
            Crie filtros personalizados para encontrar exatamente o que você precisa
          </DialogDescription>
        </DialogHeader>
        ...
        <ScrollArea className="h-[calc(100vh-220px)] pr-4">
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium">Construtor de Filtros</Label>
                <div className="flex items-center gap-2">
                  <Input
                  ...
                  <Button
                    ...
                    disabled={filterName.trim() === '' || rootFilterGroup.rules.length === 0}
