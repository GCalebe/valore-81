...
import { ShipWhell
, Mail, Lock, Eye, EyeOff, Heart, Sparkles } from 'lucide-react';
import { z } from 'zod';
        ...
        return (
          <>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full button-hover-effect bg-Valore-gold hover:bg-amber-500 text-Valore-blue dark:text-gray-900 font-bold py-3 px-4 rounded-md flex items-center justify-center transition-all duration-300 animate-slide-up"
              style={{ animationDelay: '0.6s' }}
            >
              {isLoading ? (
                <div className="h-5 w-5 border-2 border-Valore-blue dark:border-gray-900 border-t-transparent rounded-full animate-spin mr-2"></div>
              ) : (
                <ShipWhell
  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        <div className="absolute inset-0 backdrop-blur-md bg-Valore-blue/30 dark:bg-gray-900/50"></div>
      </div>
      
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-10">
        <div className="absolute top-[10%] left-[5%] w-28 h-28 rounded-full bg-Valore-gold dark:bg-amber-500 opacity-20 animate-float"></div>
        <div className="absolute bottom-[20%] right-[10%] w-40 h-40 rounded-full bg-Valore-gold dark:bg-amber-500 opacity-10 animate-float" style={{ animationDelay: '1s' }}></div>
        <div className="absolute top-[40%] right-[20%] w-24 h-24 rounded-full bg-Valore-gold dark:bg-amber-500 opacity-15 animate-float" style={{ animationDelay: '2s' }}></div>
        
        <ShipWhell
 className="absolute top-[15%] right-[25%] w-16 h-16 text-white opacity-10 animate-bounce" style={{ animationDelay: '1.5s' }} />
        <ShipWhell
 className="absolute bottom-[30%] left-[15%] w-20 h-20 text-white opacity-10 animate-float" style={{ animationDelay: '2.5s' }} />
        <ShipWhell
 className="absolute top-[60%] right-[10%] w-12 h-12 text-Valore-gold dark:text-amber-500 opacity-15 animate-pulse" style={{ animationDelay: '0.5s' }} />
        <Heart className="absolute top-[25%] left-[30%] w-10 h-10 text-Valore-gold dark:text-amber-500 opacity-15 animate-pulse" style={{ animationDelay: '1.2s' }} />
