...
export const useDocuments = () => {
  ...
  const handleDeleteDocument = async (id: number, title: string) => {
    try {
      ...
      const response = await fetch(
        "https://webhook.comercial247.com.br/webhook/excluir-arquivo-rag",
        {
          method: "POST",
          headers: {
  ...
  const clearAllDocuments = async () => {
    try {
      console.log(
        "Enviando solicitação para excluir toda a base de conhecimento",
      );

      const response = await fetch(
        "https://webhook.comercial247.com.br/webhook/excluir-rag",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(
          `Erro ao limpar a base de conhecimento: ${response.statusText}`,
        );
      }
  ...
  const uploadFileToWebhook = async (file: File, category: string) => {
    try {
      ...
      console.log(
        "Enviando arquivo para o webhook:",
        file.name,
        "categoria:",
        category,
      );
      ...
      const response = await fetch(
        "https://webhook.comercial247.com.br/webhook/envia_rag",
        {
          method: "POST",
          body: formData,
        },
      ...
      toast({
        ...
        description:
          "Não foi possível enviar o documento para o sistema de conhecimento.",
        variant: "destructive",
      });
