...
export function useUTMTracking(selectedCampaign: string = 'all', selectedDevice: string = 'all') {
  ...
  const refetchUTMData = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: "Problema ao buscar as métricas UTM. Usando dados de exemplo.",
        variant: "destructive"
      });
      setMetrics(mockUTMMetrics);
    } finally {
