...
export const useContactsService = () => {
  const fetchAllContacts = async (): Promise<Contact[]> => {
    try {
      ...
      // Se a view existir, busca os dados dela
      if (!viewCheckError) {
        ...
        console.warn('View v_clients_complete not found, using fallback:', viewCheckError);
      }
      
      // Fallback to contacts table with manual joins
      console.info('Using fallback: fetching from contacts table with manual joins');
