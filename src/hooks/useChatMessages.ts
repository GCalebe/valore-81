...
const generateMockMessages = (sessionId: string): ChatMessage[] => {
  ...
  if (client.clientType === 'pessoa-juridica') {
    ...
    messages.push({
      role: 'human',
      content: client.clientObjective || 'Preciso de uma consultoria personalizada para minha situação.',
      timestamp: '14:33',
      type: 'human'
    });
  ...
  if (client.kanbanStage === 'Negociação') {
    messages.push({
      role: 'human',
      content: 'Gostei da proposta que vocês enviaram. Podemos conversar sobre os valores?',
      timestamp: '15:20',
      type: 'human'
    });
  ...
  if (client.kanbanStage === 'Projeto cancelado – perdido') {
    messages.push({
      role: 'human',
      content: 'Infelizmente precisamos cancelar o projeto por questões orçamentárias.',
      timestamp: '16:10',
      type: 'human'
    });
  ...
  if (client.lastMessage && client.lastMessage !== messages[messages.length - 1]?.content) {
    messages.push({
      role: 'human',
